#ifndef STUBS_H
#define STUBS_H

#include <stdint.h>
#include <stddef.h>

void FUN_000003ba(void *dest, int size);
void FUN_00001758(void);
void FUN_00002c4c(void *buf, int size);
void FUN_00000288(void *dest, const void *src, int size);
void FUN_00005460(int delay);
int FUN_00000230(const void* buf1, const void* buf2, int size);
void FUN_000053ec(int count);
void FUN_0000529c(uint8_t, uint8_t, uint8_t);
void FUN_00005224(uint8_t, uint8_t);
void FUN_00005200(void*, uint8_t);
void FUN_00004dac(void*, uint16_t);
void FUN_00004f46(void*, uint8_t);
void FUN_00004a90(void);
void FUN_00003e60(void);
void FUN_00003a6c(void);
void FUN_00003888(void);
void FUN_0000350c(void*);
void FUN_00001cac(void);
void FUN_00005510(uint8_t);
void FUN_000054ac(uint16_t);
void FUN_0000531c(uint8_t, uint8_t, void*);
void FUN_00004e1a(void*, uint16_t, uint8_t);
void FUN_00004d10(void*);
void FUN_00004cf6(void*, uint8_t);
void FUN_000049e8(void*);
void FUN_00004928(uint8_t);
void FUN_000047c0(void*);
void FUN_000046a4(void*);
void FUN_00004504(void*);
void FUN_000043d0(void*);
void FUN_000042b8(void*);
void FUN_00004264(void);
void FUN_00003d44(void);
void FUN_00003cc0(void);
void FUN_00003be0(void);
void FUN_00003b50(void);
void FUN_00003b00(void*);
void FUN_0000232c(uint8_t);
void FUN_000022e0(uint8_t, uint16_t);
void FUN_00002220(uint8_t, uint8_t);
void FUN_000021bc(uint8_t);
void FUN_00002174(void);
void FUN_00002148(void);
void FUN_0000056c(void);
void FUN_00001d84(uint32_t, uint32_t);
void FUN_000012b6(void*, void*);
void FUN_00001a54(void*);
void FUN_000014a0(void);
void FUN_00005a44(void);
void FUN_0000524c(void *dest, void *src, int size);
void FUN_00001480(void);
void hw_send_command_and_wait(uint8_t cmd, uint8_t len, void* data);


#endif // STUBS_H
